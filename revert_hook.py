...
old_hook = """function useFirmEmployeeConnections(crd: string, enabled: boolean, detailJson: string) {
	const [state, setState] = useState<{ current: any[]; previous: any[]; loading: boolean } | null>(null);

	React.useEffect(() => {
		if (!enabled || !crd || !detailJson) {
			setState(null);
			return;
		}
	...
	}, [crd, enabled, detailJson]);

	return state;
}"""

new_hook = """function useFirmEmployeeConnections(crd: string, enabled: boolean) {
	const [state, setState] = useState<{ current: any[]; previous: any[]; loading: boolean } | null>(null);
...
old_call = """const employeeConnections = useFirmEmployeeConnections(parsedKey?.crd || '', parsedKey?.type === 'firm', detailJson);"""
new_call = """const employeeConnections = useFirmEmployeeConnections(parsedKey?.crd || '', parsedKey?.type === 'firm');"""
